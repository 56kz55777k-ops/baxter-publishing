...
*(Closed since the last revision: **PR #2 — merged 2026-08-22, commit `43b5335`**, post-merge CI and Vercel production deployment both green. Previously closed: PR #1 — merged 2026-08-12, commit `7826384`; the GitHub Actions verification hold — cleared.)*
...
2. `cd ~/Desktop/baxter-app && git fetch && git status` — expect **`main` at `43b5335` or later** (D-033 is merged; nothing is pending), clean tree apart from four pre-existing untracked folders (`Resend Documentation/`, `Vercel Documentation/`, `brand/`, `baxter-slice6-smoke-test.md`). Branches present: `slice-a-native-publishing` (merged record) and `amendment/d-033-publication-bleed` (PR #2). Check `gh pr list` — expect none open.
...
4. Check Ben's queue (§8). The only engineering gate is the A4/square geometry ruling; once it lands, begin Slice B per §9 on a fresh `slice-b-*` branch off `main`, contracts open beside the code. Do not start Slice B features before that ruling.
