...
**Status: RULING ACCEPTED by Ben, 2026-09-18; IMPLEMENTATION ACCEPTED by Ben, 2026-09-19.** The two acceptances are separate events and are not to be collapsed, exactly as for D-033.

**Merged.** [PR #3](https://github.com/56kz55777k-ops/baxter-publishing/pull/3) was merged into `main` on **2026-09-19T14:50:45Z** as merge commit **`b6b2f72a2e8ecfe2c25c7926b157b2c692c7fc39`** (parents `699f009` + the reviewed head `6d6f94305f2b9eb6839227a7944676e90382b01e`). The merge commit's tree is `586467e0d36e585111ed2eefb36eefacf82173b6` — byte-identical to the reviewed tree, so the merge introduced nothing of its own.

**Verification, recorded with the states kept distinct.** Hosted CI **passed** on the PR head (run `35449531226`, event `pull_request`, conclusion `success`) and again on the merge commit itself (run `35449976768`, event `push`, conclusion `success`) — every step green: `npm ci` · typecheck · test-tree typecheck · lint · test · build · bundle budget. The Vercel **preview** deployment on the reviewed head succeeded (`6515309467`), and the **production** deployment on the merge commit succeeded (`6542181246`); `https://baxter-publishing-web.vercel.app` and `/publications` both return HTTP 200. The local battery on the merged tree: 15 suites / 124 tests, typecheck, test-tree typecheck, lint, build, bundle budget PASS, preflight harness 6/6 — note the preflight harness is a local-only gate and is not part of the hosted workflow. Branch `amendment/d-034-publication-margins` is retained as the reviewed-head record, not deleted.
